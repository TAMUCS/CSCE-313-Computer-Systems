...
// Server Channel Type 
RequestChannel* chanTServer(const std::string i, byte_t* buf=((byte_t*)"control")) {
	if (i == "f") { return (new FIFORequestChannel(buf, RequestChannel::SERVER_SIDE)); }
	if (i == "q") { return (new MQRequestChannel(buf, RequestChannel::SERVER_SIDE, buffercapacity)); }
	if (i == "s") { return (new SHMRequestChannel(buf, RequestChannel::SERVER_SIDE, buffercapacity)); }
	return nullptr;
}
...
void process_newchannel_request (RequestChannel* _channel) {
	...
	RequestChannel* data_channel(chanTServer(i, (byte_t*)new_channel_name.c_str()));
...
int main (int argc, char* argv[]) {
	...
	while ((opt = getopt(argc, argv, "m:i:")) != -1) {
		switch (opt) {
			...
			case 'i':
				...
				break;
			default:
				throw Oopsie_Daisies("Server: getopt()");
		}
	}
	...
	RequestChannel* control_channel(chanTServer(i));
